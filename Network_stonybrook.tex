...
Motivation
\begin{itemize}
\item How people act collectively under uncertainty?
\begin{itemize}
...
%\textcolor{blue}{-} network-monitoring & [Renault and Tomala 2004], etc & \alert{This paper}, etc
%\end{tabular}
%\end{table}
%}
%
...
\frame{
  \frametitle{Equilibrium construction for \alert{$k<n$}}
For simplicity, assume $T$ is fixed, commonly known, and independent from states. 
%\begin{center}
%\begin{tikzpicture}%[
%%    thick,
%%    >=stealth',
...
\item Idea:
\begin{itemize}
\item Suppose players can transmit information by ``talking'' within \alert{$\hat{T}$ rounds}, where there are multiple periods in each round,  and then play a one-shot game.
%\begin{enumerate}
...
%
%\begin{itemize}
%\item \underline{$\hat{T}$ is a big number.}
...
%\[ \text{ for } 2\leq t \leq $\hat{T}$\text{ , }M^{t}_i=\{f|f:\prod_{j\in G_i}M^{t-1}_j\rightarrow W\}\cup\{\emptyset\} \]
...
\begin{frame}
  \frametitle{$k$-Threshold game augmented by $\hat{T}$-round \alt<3->{costly talk}{cheap talk}}

\onslide<3->{ If there is a fixed cost $\epsilon$ to send the letter...}

\begin{itemize}

\item <1-> Off-path strategy can be constructed
\begin{itemize}
\item <2-> If S-type 4 (or 5) make detectable deviation \alt<3->{(e.g. wrong sentence, playing $\emptyset$)}{(e.g. wrong sentence)}

$\Rightarrow$ others play \alt<3->{$\emptyset$}{\textbf{n}} and then \textbf{n}.
...
\onslide<4>{
\alert{So}, when \textcolor{red}{$\epsilon$ is small enough and $\hat{T}$ is large enough}, a WPBE can be constructed \alert{when $\epsilon$ is independent from messages}.}

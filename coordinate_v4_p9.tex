...
\newcommand{\Kappa}{\mathbb{COORD}}
\newcommand{\Omicron}{\mathbb{RPT}}
\newcommand\omicron{o}
...
\end{lemma}
\begin{proof}
On the equilibrium path, $j$ can only play $\langle I^t_j \rangle$, $\langle \textbf{all stay} \rangle$ or $\langle 1 \rangle$. Moreover, there are certain $\langle I^t_j \rangle$ with which $j$ can play since players are indexed by commonly known prime numbers. This implies that there are only certain periods in which $j$ can play \textbf{r}, and there periods are not overlapped with what $i$ plays.
...
In the event of $\#[R](\theta)\geq k$, suppose no one satisfies \textbf{k-1 Condition}, then $\langle 1 \rangle$ dose not happen on the path. $i$ play $\langle 1 \rangle$ only if he knows $j$ is k-1 Condition or Failure Condition before he plays $I^t_i$. $i$ can only infer that through $i$'s neighbor $i^{'}$. This means that the minimum of $\times I^t_{i^{'}}$ is larger than $\times I^t_i$. Since $i^{'}$ is not first order, $i^{'}$ play $\langle 1 \rangle$ only if $i^{''}$ whose $\times I^t_{i^{''}}$ is larger than $\times I^t_{i^{'}}$. Continue this process, there must be $j$ so that $\times I^t_{j}$ is larger than $\times I^t_{j^{'}}$ for all $j^{'}\in G_j$. Note that it is possible that $j$ satisfies k-1 condition since $\#R[\theta]\geq k$. This $j$ will play $I^t_j$ since he is not first order. But then his neighbor will not play $\langle 1 \rangle$, and then no one will play $\langle 1 \rangle$ on the path.

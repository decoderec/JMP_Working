...
\noindent \textbf{$k-1$ Condition for the $m$-th period in the $t$-block reporting phase} 
\begin{enumerate}
...
For every Rebel $i$, define $\Theta^{k-1}_i$ and $\Theta^{fail}_i$ as follows.
\[\Theta^{fail}_i=\{\theta:\text{$i$ is inactive }\Leftrightarrow\#[R](\theta)<k\}\]
\[\Theta^{k-1}_i=\{\theta:\text{$i$ is active }\Rightarrow \text{$i$ satisfies $k-1$ Condition}\}\]
\[\Theta^{critical}=\bigcup_{i\in [R](\theta)}(\Theta^{fail}_i\cup\Theta^{k-1}_i)\]

$i$ is certain that \textit{$\theta$ is $k-1$ critical} if
\[\beta_i(\Theta^{critical}|)=1\]
...
%\begin{lemma}
%Suppose Rebel $i$ follows an APEX equilibrium $\tau^{*}$ till period $s$, in which $0<\beta^{\tau^{*}}_{G_i}(\#[R](\theta)\geq k|h^s_{G_i})\leq 1$ and $s<T^{\theta}_{\tau^{*}}$. Let $\mathbf{supp}(\beta_i)$ be the support of $\beta^{\tau^{*}}_{G_i}(\#[R](\theta)\geq k|h^s_{G_i})$ and $\tau|^s_{\tau^{*}}=(\tau_i,\tau_{-i})|^s_{\tau^{*}}$ be a continuation strategy. If, conditional on $E_1\subseteq \mathrm{supp}(\beta_i)$, $\tau|^s_{\tau^{*}}$ generates the stage payoff of 0 after $\infty> s^{'}>s$, then Rebel $i$ will not deviate to $\tau|^s_{\tau^{*}}$ if $\delta$ is sufficiently high. 
%\end{lemma}
%\begin{proof}
...
%\begin{lemma}
%Suppose Rebel $i$ follows a strategy $\hat{\tau}=(\hat{\tau}_{i},\tau^{*}_{-i})$ that has not been detected as a deviation till period $s$. If the continuation strategy $\tau|^s_{\hat{\tau}}$ makes detectable deviation after some $s^{'}$, Rebel $i$ will not deviate to a continuation strategy $\tau|^s_{\hat{\tau}}$ that makes detectable deviation if $\delta$ is sufficiently high. 
%\end{lemma}
%\begin{proof}
%$0<\beta^{\hat{\tau}}_{G_i}(\#[R](\theta)\geq k|h^s_{G_i})< 1$. 
%\end{proof}
...
%
%
%\begin{lemma}%Along with the equilibrium path
...
\begin{lemma} When $\#[R](\theta)\geq k$, on the equilibrium path,
\begin{enumerate}
\item a Rebel $i$ plays $\langle 1 \rangle$ only if $i$ is pivotal or $i$ is certain that the state is k-1 critical.
\item Suppose $i$ is active. At the $m$-th period in the $t$-block reporting phase, if $i$ has not played $\langle I^t_i \rangle$ and if $i$ is certain that $\theta$ is $k-1$ critical, then
\begin{enumerate}
\item either $i$ will not play $\langle I^t_i \rangle$, or there exists $j\in G_i$ who does not play $I^t_j$
\item if $i$ does not satisfies $k-1$ Condition, then there exists $j\in G_i$ such that the \textbf{revolt} in $\langle I^t_i \rangle$ is played after the \textbf{revolt} in every possible $\langle I^t_j \rangle$.
\item if $i$ does not satisfies $k-1$ Condition, then no $j\in G_i$ is pivotal.
\end{enumerate}
\item if $\langle 1 \rangle$ is played in the $t$-block, then there is a Rebel who is certain that the state is critical at some period in the $t$-block.
\end{enumerate}
\end{lemma}
\begin{proof}
On the equilibrium path, for any Rebel $i$, there are finitely possible $\langle I^t_i \rangle$ with which $i$ is allowed to play. This is because the network is commonly known and players are indexed by commonly known prime numbers. As a consequence, $\langle I^t_i \rangle$ is played at certain periods and only $i$ can play $\langle I^t_i \rangle$.

%On the equilibrium path, $j$ can only play $\langle I^t_j \rangle$ or $\langle x \rangle$, where $1\leq x \leq k$. Moreover, there are certain $\langle I^t_j \rangle$ with which $j$ can play since players are indexed by commonly known prime numbers. This implies that there are only certain periods in which $j$ can play \textbf{r}, and there periods are not overlapped with what $i$ plays.
%
%$i$ cannot directly observe what $j^{'}$ play and thus infer $j^{'}$'s play through $j$'s play. In case $j$ has played $\langle I^t_j \rangle$, $i$ excludes $j$ is a player satisfying \textbf{k-1 Condition}, and cannot infer whether or not there is $j^{'}$ who satisfies \textbf{k-1 Condition} or \textbf{Failure Condition}. This is because it remains a possibility that $j^{'}$ is not \textbf{k-1 Condition} and therefore does not influence $j$'s behavior. Thus, before $j$ plays possible \textbf{r}, $i$ cannot infer whether or not there is a player satisfying \textbf{k-1 Condition} or \textbf{Failure Condition}. Continue this reasoning, $i$ can only make inference from $j$'s behavior when $j$ does not play all possible $\langle I^t_j \rangle$. 
%
%$j$ does not play $\langle I^t_j \rangle$ only if $j$ is inactive or has been influenced by his neighbor. $i$ then checks all possibility that lead $j$ does not play $\langle I^t_j \rangle$.
\begin{enumerate}
\item
\item 
\begin{enumerate}
\item Note that $i$ is certain that $\theta$ is $k-1$ critical if $\beta_i(\bigcup_{i\in I}\Theta^{k-1}_i\cup\Theta^{fail}_i|)=1$. If $\beta_i(\bigcup_{i\in I}\Theta^{fail}_i|)=1$, then $i$ will play \textbf{stay} afterwards. On the other hand, if $\beta_i(\bigcup_{i\in I}\Theta^{k-1}_i|)>0$, then some Rebel satisfies $k-1$ Condition. Suppose $i$ he himself satisfies $k-1$ Condition, then $i$ will play $\langle 1 \rangle$. The argument proceeds and will be proved by contradiction when $i$ does not. Suppose every active Rebel $j\in G_i$ has played $I^t_j$. Since $i$ is active, there are some Rebels in $I^t_i$ but outside $I^t_j$. It implies there are $n\geq 2$ Rebels in $TR_{ji}$. Since $j$ has played $I^t_j$, $j$ does not satisfies $k-1$ Condition. Consider a $\bar{\theta}$ such that $n\geq 2$ Rebels are in $TR_{ji}$ and that $k-n$ Rebels are in $TR_{ij}$. At $\bar{\theta}$, no Rebel satisfies $k-1$ Condition in $TR_{ij}$. However, by the full support on strong connectedness, $\bar{\theta}$ is on the support of $i$'s believe. It violates $\beta_i(\bigcup_{i\in I}\Theta^{k-1}_i\cup\Theta^{fail}_i|)=1$, and hence $i$ is uncertain the state is $k-1$ critical.

%Since $i$ is not $\theta$-pivotal, $i$ cannot reach all nodes by $t+1$ edges. This implies it is possible that $X^{t+1}_i\geq k$ even if $I^{t+1}_i<k$. Notice that $i$ plays $\langle 1 \rangle$ by assumption. It implies $X^{t+1}_i\geq k$ at every moment $i$ plays. Consider a $\hat{\theta}$ consistent with that $X^{t+1}_i\geq k$, that $n\geq 2$ Rebels are in $TR_{ji}$ and that $k-n$ Rebels are in $TR_{ij}$. By the full support on strong connectedness, $\hat{\theta}$ is on the support of $i$'s belief. However, no Rebel satisfies $k-1$ Condition in $TR_{ij}$ at $\hat{\theta}$, and hence $i$ is uncertain whether or not there exists a $k-1$ Condition Rebel in $TR_{ij}$. 

\item By (a), if $i$ is certain that the state is critical, there exists a $j\in G_i$ has not played $\langle I^t_j \rangle$ at the moment $i$ starts to play $\langle 1 \rangle$. However, $i$ does not know $\langle I^t_j \rangle$ before it has been played. Therefore, $i$ can be certain that there exists a $k-1$ Condition Rebel in $TR_{ij}$ only if $j$ has not played \textit{every possible} $\langle I^t_j \rangle$. It happens when $\langle I^t_i \rangle$ is played after every possible $\langle I^t_j \rangle$. 

\item This is because a pivotal player does not play $I^t_p$ no matter $p$ satisfies $k-1$ Condition or not. By the full support on strong connectedness, 
\item Since $i$ is certain that the state is critical but he does not satisfies $k-1$ Condition, he is certain that there exists $p$ who satisfies $k-1$ Condition. $p\in TR_{ij}$ for $j\in G_i$ by that the network is connected. 
\item Since the prior has full support on strong connectedness, and players are Bayesian on the path, a state cannot happen if there exists a player who believes that cannot happen. By definition, a player is certain that the state is critical if and only if there exists a Rebel who satisfies $k-1$ Condition. Thus   
\end{enumerate}


\item Suppose $i$ is active, not pivotal, and certain that either there is a Rebel satisfying $k-1$ Condition or $\#[R](\theta)<k$. Since $i$ does not satisfy $k-1$ Condition, $i$ is certain that either some other Rebels satisfies $k-1$ Condition or $\#[R](\theta)<k$ at the moment $i$ starts to play $\langle 1 \rangle$ as the replacement of $\langle I^t_i \rangle$. I claim that, at this moment, if $i$'s neighbor $j$ has played $I^t_j$ then $i$ is uncertain whether or not there exists a $k-1$ Condition Rebel in $TR_{ij}$. To show it, recall $i$ is active and not pivotal. Since $i$ is active, there is some Rebel $j^{'}$ in $I^t_i$ but outside $I^t_j$. It implies there are $n\geq 2$ Rebels in $TR_{ji}$. Since $j$ has played $I^t_j$ and therefore $\#I^t_j\leq k-2$. Since $i$ is not $\theta$-pivotal, $i$ cannot reach all nodes by $t+1$ edges. This implies it is possible that $X^{t+1}_i\geq k$ even if $I^{t+1}_i<k$. Notice that $i$ plays $\langle 1 \rangle$ by assumption. It implies $X^{t+1}_i\geq k$ at every moment $i$ plays. Consider a $\hat{\theta}$ consistent with that $X^{t+1}_i\geq k$, that $n\geq 2$ Rebels are in $TR_{ji}$ and that $k-n$ Rebels are in $TR_{ij}$. By the full support on strong connectedness, $\hat{\theta}$ is on the support of $i$'s belief. However, no Rebel satisfies $k-1$ Condition in $TR_{ij}$ at $\hat{\theta}$, and hence $i$ is uncertain whether or not there exists a $k-1$ Condition Rebel in $TR_{ij}$. 

In other words, $i$ can be certain that there exists a $k-1$ Condition Rebel in $TR_{ij}$ only if $j$ has not played $\langle I^t_j \rangle$ at the moment $i$ starts to play $\langle 1 \rangle$. Nevertheless, $i$ does not know $\langle I^t_j \rangle$ before it has been played. Therefore, $i$ can be certain that there exists a $k-1$ Condition Rebel in $TR_{ij}$ only if $j$ has not played \textit{every possible} $\langle I^t_j \rangle$. It happens when $\langle I^t_i \rangle$ is played after every possible $\langle I^t_j \rangle$. 

Since $\#[R](\theta)\geq k$, there is a $j$ who is active.

$i$ is certain that $j$ is not a pivotal Rebel who will play $\langle x \rangle$.  If it were not the case, since the pivotal player would necessarily satisfies $k-1$ Condition. Take a $\theta$ that $j$ is not pivotal. Conditional on this $\theta$, since $j$ is not pivotal but $j$ does not play $I^t_j$, $j$ satisfies $k-1$ Condition. $j$ is certain that $j^{'}$ is in $TR_{jj^{'}}$. It again means that $\langle I^t_j \rangle$ is played after every possible $\langle I^t_{j^{'}} \rangle$. Continue this process, for every $\hat{j}$ there is a $\hat{\hat{j}}$ whose $I^t_{\hat{j}}$ is larger than $I^t_{\hat{\hat{j}}}$. It is impossible since the network is finite and therefore there is an upper bound of $I^t_j$. 

\item The proof is done by contradiction. Suppose no Rebel satisfies $k-1$ Condition at any period in the $t$-block. Let $i$ be a Rebel who plays $\langle 1 \rangle$. 

If $i$ is $k-1$ pivotal, then $I^t_i=k$, and therefore $i$ satisfies k-1 Condition at the first period in $\Omicron^t$. It contradicts that no Rebel satisfies $k-1$ Condition. 

If $i$ is $\theta$-pivotal but not the free-rider, $i$ plays $\langle 1 \rangle$ only when $i$ is $k-1$ pivotal. Therefore $i$ satisfies k-1 Condition at the first period in $\Omicron^t$. It contradicts that no Rebel satisfies $k-1$ Condition. 

If $i$ is $\theta$-pivotal and the free-rider, then every $i$'s active Rebel neighbor, says $j$, plays $I^t_j$ unless $j$ is $k-1$ pivotal or satisfies $k-1$ condition. If $j$ is $k-1$ pivotal or satisfies $k-1$ condition, then a contradiction occurs. If every $j$ plays $I^t_j$, then $I^{t+1}_i\geq k$ by that $\#[R](\theta)\geq k$ and $[R](\theta)\subseteq I^{t+1}_i$. Since $\langle I^t_j \rangle$ is played before $\langle 1 \rangle$. $i$ satisfies $k-1$ Condition at some period in $\Omicron^t$. It also contradicts that no Rebel satisfies $k-1$ Condition. 

If $i$ is 1-edge-$\theta$-pivotal, then $[R](\theta)\subseteq I^{t+2}_i$ by the definition of 1-edge-$\theta$-pivotal and $\#(I^{t+1}_i\setminus G^{t+1}_j)=1$ by that $i$ plays $\langle 1 \rangle$. Since the other 1-edge-$\theta$-pivotal Rebel $j$ does not satisfies $k-1$ Condition at any period in $\Omicron^t$, therefore $\#I^{t+1}_j<k-1$, and hence $\#I^{t+2}_j=\#(I^{t+1}_i\cup I^{t+1}_j)<k$. Since $[R](\theta)\subseteq I^{t+2}_j$, this implies $\#[R](\theta)< k$. It contradicts to $\#[R](\theta)\geq k$. 

%Since no Rebel satisfies \textbf{k-1 Condition}, a Rebel $i$ plays $\langle 1 \rangle$ only in two circumstances. One circumstance is when Rebel $i$ is 1-edge-$\theta$-pivotal and plays $\langle 1 \rangle$. In this case, $[R](\theta)\subseteq I^{t+2}_i$ and $\#(I^{t+1}_i\setminus G^{t+1}_j)=1$. Since the other 1-edge-$\theta$-pivotal Rebel $j$ does not satisfies \textbf{k-1 Condition}, therefore $\#I^{t+1}_j<k-1$, and hence $\#I^{t+2}_i=\#(I^{t+1}_i\cup I^{t+1}_j)<k$. This however contradicts to $\#[R](\theta)\geq k$. 



the only Rebels who satisfies $k-1$ Condition, then these Rebels must satisfies $k-1$ Condition when $\#[R](\theta)\geq k$. This is because, if all of these Rebels do not satisfy that condition,    

$i$ can only infer that through $i$'s neighbor $i^{'}$. Since $\langle 1 \rangle$ is played later than any $\langle I^t_{i^{'}} \rangle$ by definition, this means that all possible $\langle I^t_{i^{'}} \rangle$ is not played before $\langle I^t_i \rangle$. Since $i^{'}$ does not satisfies k-1 Condition, $i^{'}$ play $\langle 1 \rangle$ only if $i^{''}$ whose $\times I^t_{i^{''}}$ is larger than $\times I^t_{i^{'}}$. Continue this process, there must be $j$ so that $\times I^t_{j}$ is larger than $\times I^t_{j^{'}}$ for all $j^{'}\in G_j$. Note that it is possible that $j$ satisfies k-1 condition since $\#R[\theta]\geq k$. This $j$ will play $I^t_j$ since he is not first order. But then his neighbor will not play $\langle 1 \rangle$, and then no one will play $\langle 1 \rangle$ on the path.

%in the event of $\#[R](\theta)\geq k$, if there is $i$ so that $\#X^t_i<k$, then $i$ will play \textbf{s} afterwards and therefore all player eventually play \textbf{stay} after $\Kappa^{t+1}$.   

In the event of $\#R[\theta]\geq k$, if $\#I^t_i<k$ and $\#I^{t+1}_i$ for all $i$, then there is no $\langle 1 \rangle$ on the path. This is because no one satisfies k-1 condition. This is because $I^{t+1}_i<k$, $\#R[\theta]\geq k$, and therefore there exists a Rebel outside $I^{t+1}_i$ for all $i$. Bute then no $i$ can be certain whether or not $\#[R](\theta)\geq k$ given his neighbor's $I^{t}_{i^{'}}$. Thus no one is k-1 condition. 

In the event of $\#R[\theta]\geq k$, $\langle 1 \rangle$ happens only when there is a Rebel $i$ whose $I^t_i<k$ and $I^{t+1}_i\geq k$. 

In the event of $\#R[\theta]<k$, if all player play $\langle I  \rangle$, then all player play \textbf{stay} whenever there is a player $j$ $X^t_j<k$. If there is $\langle 1 \rangle$, there is a player find out $X^{t+1}<k$ immediately after $\Omicron^t$. 
\end{enumerate}

\end{proof}

\begin{lemma} Suppose Rebel $i$ follows the constructed APEX equilibrium $\tau^{*}$ till period $s$ and $s<T^{\theta}_{\tau^{*}}$. 

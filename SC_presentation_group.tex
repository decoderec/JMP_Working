...
\date{Social Network Research Group}
...
\item If so, how people act collectively? How do they form an uprising, a currency attack,...,etc. 
%\begin{itemize}
%\item Ex.: protest, joint investment, etc.
%\end{itemize}
\end{itemize}
...
%\\
%\textcolor{blue}{imperfect monitoring} & [Aumann and Maschiler 1990], etc & [Fudenberg and Yamamoto 2010,2011]\\
%&& [Wiseman 2012], [Yamamoto 2014], etc.\\
...
\item Players play a collective action---a protest---many many many...many periods with discount factor.
\end{enumerate}
...
%\begin{itemize}
%\item ~[Aumann and Maschiler 1990], [Miyahara and Sekiguchi 2013], [Wolitzky 2013], etc.
%\end{itemize}
\end{itemize}
...
\frame{
  \frametitle{Result 1: APEX for $k=L$}

\begin{theorem}[\alert{$k=L$}]

If $k=L$, then an APEX equilibrium exists whenever discount factor is sufficiently high.
\end{theorem}
...
\item Note that B-type will not reveal information.
...
\frame{
  \frametitle{Result 2: APEX for $k<L$}

\begin{theorem}[\alert{$k< L$}]
If $k<L$, then if network is a \alert{tree}, if prior has \alert{full support on strong connectedness}, then an APEX WPBE {exists} whenever discount factor is sufficiently high.
\end{theorem}
...
%  \pause
%  \item Let discount factor be sufficiently high to impede deviation.
%  \pause
...
\frame{
  \frametitle{Equilibrium construction for {$k<L$}}
...
\item Challenges:
\begin{itemize}
\item Only two actions---$\{\textbf{n},\textbf{p}\}$--- used for transmit relevant information.
...
\begin{enumerate}
\item We first consider fixed $T$.
\item Then allow indeterministic $T$.
\end{enumerate}
...
\frame{
  \frametitle{Equilibrium construction for {$k<L$}}
Assume $T$ is fixed, commonly known, and independent from states. 
%\begin{center}
%\begin{tikzpicture}%[
%%    thick,
%%    >=stealth',
...
%\begin{itemize}
%\item If a player make a detectable deviation \alt<2->{(e.g. wrong sentence, playing $\emptyset$)}{(e.g. wrong sentence)}
%
%$\Rightarrow$ the neighbor who detects that deviation plays \alt<2->{$\emptyset$}{\textbf{n}} and then \textbf{n}.
...
%\begin{itemize}
%\item There are \underline{multiple neighboring players} who can assure that \underline{they can learn the relevant information given others' truthful announcement} \underline{by knowing the network structure}. 
%\end{itemize}
%
...
%}
%{
%At \textcolor{blue}{$t=1$}, let S-type 4 be free rider.
%\begin{table}[h]
%\begin{tabular}{ll l}
%S-type 4 & $(\overbrace{\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}}}^{L=8})$\\
%S-type 5 & $(\overbrace{\textbf{n},\textbf{n},\textbf{n},\alert{\textbf{p}},\alert{\textbf{p}},\alert{\textbf{p}},\textbf{n},\alert{\textbf{p}}}^{L=8})$ \\
...
%Example of solving Free Rider Problem (Suppose $\epsilon$ \alert{is decreasing} when announcing \alert{more} S-types in the \alert{$1^{st}$} round; After $1^{st}$ round, $\epsilon$ is fixed):
%\begin{itemize}
%%\item $k=5$, $n=8$ and \underline{let $T=\alert{4}$}.
%\item $k=5$, $n=8$ and let $T=3$.
%\item $G$ and $\theta$=
%\begin{center}
%\begin{tikzpicture}[scale=0.7]
...
%  \framesubtitle{Free Rider Problem}
%  
%\begin{lemma} If the network is a tree, then the Free Rider Problem only occurs between two players, and they will commonly know that they are in this problem.
%\end{lemma}  
%\begin{itemize}
...
%
%\draw[->] (0,0) -- (8,0); %coordinate[label = {below:$x$}] (xmax);
%\draw[-] (7,0) -- (7, 0.1) node [above]{$T(?)$};
%%\draw (7,-0.1)  node [below]{$?$};
%\draw[-] (0,0) -- (0, 0.1) node [above]{$1$};
%\draw[-] (1,0) -- (1, 0.1) node [above]{$2$};
%\draw[-] (2,0) -- (2, 0.1) node [above]{$3$};
%\draw (4, 0.1) node [above]{$\cdot$};
%\draw (4.2, 0.1) node [above]{$\cdot$};
%\draw (4.4, 0.1) node [above]{$\cdot$};
%%\draw[-] (3.4,0) -- (3.4, 0.1) node [above]{$T-1$};
%\end{tikzpicture}
%\end{center}
%\begin{center}
%$\downarrow$
...
%\begin{tikzpicture}
%
%\draw[->] (-1,0) -- (8,0); %coordinate[label = {below:$x$}] (xmax);
%%\draw[-] (7,0) -- (7, 0.1) node [above]{$T$};
%%\draw (4,-0.1)  node [below]{$?$};
%\draw[-] (0,0) -- (0, 0.1) node [above]{$1$};
%\draw[-] (1,0) -- (1, 0.1) node [above]{$2$};
%\draw[-] (2,0) -- (2, 0.1) node [above]{$3$};
%
%\draw[-] (-1,0) -- (-1, -0.1) node [below]{$0^{'}_1$};
%%\draw[-] (-0.8,0) -- (-0.8, -0.1) node [below]{$0^{'}_1$};
%\draw (-0.6,0)  node [below]{$\cdot$};
%\draw (-0.5,0)  node [below]{$\cdot$};
%\draw[-] (-0.2,0) -- (-0.2, -0.1)  node [below]{$0^{'}_{n_0}$};
%
%\draw[-] (0.2,0) -- (0.2, -0.1) node [below]{$1^{'}_1$};
%%\draw[-] (0.4,0) -- (0.4, -0.1) node [below]{$1^{'}_1$};
%\draw (0.5,0)  node [below]{$\cdot$};
%\draw (0.6,0)  node [below]{$\cdot$};
%\draw[-] (0.8,0) -- (0.8, -0.1)  node [below]{$1^{'}_{n_1}$};
%
%\draw[-] (1.2,0) -- (1.2, -0.1) node [below]{$2^{'}_1$};
%%\draw[-] (0.4,0) -- (0.4, -0.1) node [below]{$1^{'}_1$};
%\draw (1.5,0)  node [below]{$\cdot$};
%\draw (1.6,0)  node [below]{$\cdot$};
%\draw[-] (1.8,0) -- (1.8, -0.1)  node [below]{$2^{'}_{n_1}$};
%
%\draw[-] (2.2,0) -- (2.2, -0.1) node [below]{$3^{'}_1$};
%%\draw[-] (0.4,0) -- (0.4, -0.1) node [below]{$1^{'}_1$};
%\draw (2.5,0)  node [below]{$\cdot$};
%\draw (2.6,0)  node [below]{$\cdot$};
%\draw[-] (2.8,0) -- (2.8, -0.1)  node [below]{$3^{'}_{n_1}$};
%
%
%\draw (4, 0.1) node [above]{$\cdot$};
%\draw (4.2, 0.1) node [above]{$\cdot$};
...
%\item $\Gamma^{'}=\{0^{'}_1,...,0^{'}_{l_0},1^{'}_{1},...,1^{'}_{l_1},...\}$ is the set of rounds in which the terminal round $T$ is decided. 
%\end{itemize}
%\end{frame}
%
%
%\begin{frame}
%  \frametitle{$k$-Threshold game augmented by indeterministic $T$-round costly talk}
%  %\framesubtitle{Examples of WPBE construction}
...
%    % Draw a 7,11 network
%    % First we draw the vertices
%    \foreach \pos/\name in {{(1,2)/1}, {(2,1)/2}, {(3,2)/4}, {(2,3)/3}, {(5,3)/7}, {(4,2)/5}, {(5,1)/6}, {(6,2)/8}}
%        \node[vertex] (\name) at \pos {$\name$};
%    
%%    \foreach \pos/\name in {{(2,3)/B_3}, {(5,3)/B_7}, {(4,2)/B_5}, {(5,1)/B_6}, {(6,2)/B_8}}
%%    \node[unknown vertex] (\name) at \pos {$\name$};
%    
%%    \foreach \pos/\name in {{(3,2)/S_4}, {(4,2)/S_5}}
%%    \node[selected vertex] (\name) at \pos {$\name$};
%    
%    % Connect vertices with edges 
%    \foreach \source/ \dest in {1/4, 2/4,3/4,4/5, 5/6, 5/7, 5/8}
%        \path[edge] (\source) -- (\dest) ;
%        
%\end{tikzpicture}
%\end{center}
...
%%\item In each round in $\Gamma$, the length of a non-empty sentence is $8$.
%%\item In each round in $\Gamma^{'}$, the length of a non-empty sentence is $1$. 
%\end{itemize}
%
%\end{frame}
%
...
%    % Draw a 7,11 network
%    % First we draw the vertices
%    \foreach \pos/\name in {{(1,2)/S_1}, {(2,1)/S_2}, {(3,2)/S_4}}
%        \node[vertex] (\name) at \pos {$\name$};
%    
%    \foreach \pos/\name in {{(2,3)/B_3}, {(5,3)/B_7}, {(4,2)/B_5}, {(5,1)/B_6}, {(6,2)/B_8}}
%    \node[unknown vertex] (\name) at \pos {$\name$};
...
%    
%    % Connect vertices with edges 
%    \foreach \source/ \dest in {S_1/S_4, S_2/S_4,B_3/S_4,S_4/B_5, B_5/B_6, B_5/B_7, B_5/B_8}
%        \path[edge] (\source) -- (\dest) ;
%        
%\end{tikzpicture}
%\end{center}
...
%\begin{tabular}{ll l}
%S-type 4 & $\emptyset$\\
%%S-type 5 & $(\overbrace{\alert{\textbf{p}},\textbf{s},\textbf{s},\textbf{s}}^{L=4})$\\
%S-type 1,2 & $({\textbf{n}})$
%\end{tabular}
%\end{table}
%
...
%the cost of writing a sentence & the expected payoff in some periods\\
...
%\\
%the sentence in announcing $i,...,j$ are S-types & $<\textbf{n},...,\underbrace{\textbf{p},...,\textbf{n}}_{z_i...z_j}>$\\

...
%\begin{notation}[The set of edges]
%\label{def:the_network_edge}
%\begin{eqnarray*}
%E & := &  \{\{v,v^{'}\}\in [N]^2: v\in G_{v^{'}}\land v^{'}\in G_v\}
%\end{eqnarray*}
%\end{notation}
%
%\begin{notation}[A walk from $i$ to $i^{'}$]
%A walk from $i$ to $i^{'}$, where $i,i^{'}\in N$, is a finite sequence $(v_1,...,v_n)$ such that $v_1=i$, $v_n=i^{'}$, and $\{v_{\bar{n}},v_{\bar{n}+1}\}\in E$ for all $1\leq \bar{n} \leq n-1$.
%\end{notation}
%
...
%\item For all $i\in N$, there is $\theta^{<k}_i\in\Theta^{<k}_i$ so that $i\in [R](\theta^{<k}_i)$. 
\end{enumerate}
...
%\item Since $\Theta$ is the set of all possible states, there is a $\theta\in \Theta$ so that $i\in R[\theta]$ and $\#[R](\theta)<k$. By 1.~of this lemma, $\theta\in \Theta^{<k}$. If $i$ is inactive in the $t$-block, then let $\theta^{<k}_i=\theta$ and the proof is done. If $i$ is active, the proof is done by construction this $\theta^{<k}_i$. Since $i$ is active, there exists a Rebel $j\in G_i$ so that $I^t_i\not\subseteq I^t_j$ at $\theta$. Let $\theta^{<k}_i\in \Theta$ such that $I^t_{i}=I^t_{j}$. Then $i$ is inactive at $\theta^{<k}_i$ since $I^t_i\subseteq I^t_j$. It follows that $\#[R](\theta^{<k})\leq \#[R](\theta)<k$, and therefore $\theta^{<k}_i\in \Theta^{k}_i$.
\end{enumerate}
...
%Since that, the support of $i$'s belief includes a $\theta^{\geq k}\in \Theta^{\geq k}$, and therefore includes a $\theta^{= k}\in \Theta^{= k}$ by the full support on strong connectedness. I claim that the stage payoff is at most $0$ at $\theta^{=k}$. Take any active $j\in G_i$, and consider the $TR_{ij}$. Let $\theta$ be the state at which all Rebels are in $I^t_j\cup TR_{ij}$ at $\theta$, and $[R](\theta)\subset [R](\theta^{=k})$. At $\theta$, $i$ is inactive by Lemma since $j$ is active and $I^t_i\subseteq I^t_j$. Since $i$ is active at $\theta^{=k}$, there is $r\in [R](\theta^{=k})$ but $r\not\in I^t_j\cup TR_{ij}$, therefore $[R](\theta)\subset [R](\theta^{=k})$. But then $\#[R](\theta)<k$. By lemma, all Rebels in $I^t_j\cup TR_{ij}$ at $\theta$ play \textbf{stay} eventually. By assumption, $i$ makes undetectable deviation, all Rebels in $I^t_j\cup TR_{ij}$ play \textbf{stay} eventually. Therefore $i$ gets at most $0$ at $\theta$ if he plays $\langle 0 \rangle$. 
%
%
%Suppose $i$ is certain that the state is critical.

...
Suppose Rebel $i$ follows an APEX equilibrium $\tau^{*}$ till period $s$ when $0<\beta^{\tau^{*}}_{G_i}(\#[R](\theta)\geq k|h^s_{G_i})<1$. Let $\tau|^s_{\tau^{*}}=(\tau_i,\tau_{-i})|^s_{\tau^{*}}$ be a continuation strategy. 
\begin{enumerate}[label=(\arabic*)]
\item If $\tau|^s_{\tau^{*}}$ generates a history $h^{s^{'}|s}$ in which $i$ will be uncertain about the relevant information and stop belief-updating after some period $s^{'}$, then $i$ will not deviate to $\tau|^s_{\tau^{*}}$ given sufficient high $\delta$.
\item If, conditional on $E_1\subseteq \mathrm{supp}(\beta_i)$, $\tau|^s_{\tau^{*}}$ generates the stage payoff of 0 after $\infty> s^{'}>s$, then Rebel $i$ will not deviate to $\tau|^s_{\tau^{*}}$ if $\delta$ is sufficiently high. 
\item If $s\not\in \Kappa^1$, then any Rebel has less than $k$ Rebel neighbors.
\item If $s\not\in \Kappa^1$ and $\tau|^s_{\tau^{*}}$ generates detectable deviations, then Rebel $i$ will not deviate to $\tau|^s_{\tau^{*}}$ if $\delta$ is sufficiently high. 
\end{enumerate}
...
\begin{lemma}Suppose Rebel $i$ plays $\langle \textbf{all stay} \rangle$ in $\Omicron^t$ for some $t\geq 1$ has not been detected. If there is a difference between $i$'s continuation play and that of a on-path $t$-block inactive Rebel, then at least one of $i$'s neighbors will detect this difference.
\end{lemma}
\begin{proof}
If $i$'s playing $\langle \textbf{all stay}\rangle$ has been detected. If $i$ has not been detected by $j$. Let $j$ be an active neignobor in the $t^{'}$-block. Since the network is acyclic, $i$ will not choose $textbf{r}$ before $j$ according to the equilibrium path. If all of $j$ is inactive in the $t^{'}$-block, since the network is acyclic, $j$ is certain that $\#[R](\theta)<k$, and therefore $i$ should play $\textbf{stay}$ forever. 
\end{proof}
...
\begin{proof}
%If $s\in \Omicron^t$, I use the notation $I^s_i$ to refer $I^{t+1}_i(m)$ subject to $s=m+\sum^t_{\gamma=1}(|\Kappa^{\gamma}|+|\Omicron^{\gamma-1}|)$. If $s\in \Kappa^t$, the notation $I^s_i$ will refer to $I^t_i$.


%I only consider the case when $2<k<n$ as aforementioned. Moreover, I only consider $s<T^{\theta}_{\tau^{*}}$. Furthermore, I only consider the case that $i$ still believe $\#[R](\theta)\geq k$ with positive probability, since otherwise $i$ will play \textbf{stay} forever on the equilibrium path.   
...
%If $\tau|^s_{\tau^{*}}$ leads to a history in which $i$ will never learn the relevant information, $i$ will not deviate. This has been proven by Claim~\ref{claim:uncertain}. 
%
%If $i$ is active, let us consider all possible histories $\tau|^s_{\tau^{*}}$ might lead to. Event $E$ has positive probability for all $s$. To see this, if $s\in\Kappa^1$, since $i$ is active, every neighbor of $i$ is not fully aware of Rebel neighbors of $i$; if $s\not\in\Kappa^1$ but $T^{\theta}$ has not been reached, every Rebel has less than $k$ Rebel neighbors. 
%
